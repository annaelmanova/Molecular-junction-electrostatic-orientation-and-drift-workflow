...
import ast, json, os, re, shutil, subprocess, sys, threading, traceback, webbrowser
...
DEFAULTS={
 ...
 'xyz_file':'','potential_dx_file':'','molecular_view_z_nm':'10.0',
...
2. DEVICE AND SWEEPS
The molecule and all three layer labels are user editable. Defaults reproduce the example system.

MOLECULAR FIGURES
Molecular figures are optional and are not bundled assets. Tick Generate molecular figures only when an XYZ geometry and a matching OpenDX electrostatic-potential file are available. Both files must use the same Cartesian frame as the dipole and tensors. The notebook copies these inputs into the run, rotates the geometry into the selected minimum-energy orientations at the requested coordinate, and generates the comparison figure. Validation fails if either required file is missing.
...
class App(tk.Tk):
 def __init__(self):
  ...
  self.vars={k:tk.StringVar(value=v) for k,v in DEFAULTS.items()}; self.generate_molecular_figures=tk.BooleanVar(value=False); self.run_dir=None
 ...
 def _build(self):
  ...
  self._qc_tab(nb); self._molecular_tab(nb); self._instructions(nb); self._run_tab(nb)
 ...
 def _molecular_tab(self,nb):
  f=ttk.Frame(nb,padding=18); nb.add(f,text='Molecular figures')
  ttk.Checkbutton(f,text='Generate molecular figures from XYZ and electrostatic potential',variable=self.generate_molecular_figures).grid(row=0,column=0,columnspan=3,sticky='w',pady=(0,14))
  for row,(key,label,kind) in enumerate([('xyz_file','XYZ geometry file','xyz'),('potential_dx_file','Electrostatic-potential OpenDX file','dx')],1):
   ttk.Label(f,text=label).grid(row=row,column=0,sticky='w',pady=7); ttk.Entry(f,textvariable=self.vars[key],width=68).grid(row=row,column=1,sticky='ew',padx=10)
   ttk.Button(f,text='Browse…',command=lambda k=key,t=kind:self._pick_molecular_file(k,t)).grid(row=row,column=2)
  ttk.Label(f,text='Orientation-selection coordinate z (nm)').grid(row=3,column=0,sticky='w',pady=7); ttk.Entry(f,textvariable=self.vars['molecular_view_z_nm'],width=18).grid(row=3,column=1,sticky='w',padx=10)
  ttk.Label(f,text='XYZ, DX potential, dipole, and tensors must share one Cartesian coordinate system.',foreground='#8A3B12').grid(row=4,column=0,columnspan=3,sticky='w',pady=14)
  f.columnconfigure(1,weight=1)
 def _pick_molecular_file(self,key,kind):
  types=[('XYZ geometry','*.xyz'),('All files','*.*')] if kind=='xyz' else [('OpenDX potential','*.dx'),('All files','*.*')]
  p=filedialog.askopenfilename(title='Select '+kind.upper()+' file',filetypes=types)
 ...
 def values(self):
  v={'save_png':True,'save_pdf':True,'dpi':300,'boltzmann_chunk':128,'generate_molecular_figures':bool(self.generate_molecular_figures.get())}; scalar=['active_layer_nm','plot_zmax_nm','z_step_nm','V_bi_V','delta_phi_ZnO_V','lambda_ZnO_nm','lambda_SAM_nm','temperature_K','orientation_temperature_K','molecular_view_z_nm']
  ...
  v['xyz_file']=self.vars['xyz_file'].get().strip(); v['potential_dx_file']=self.vars['potential_dx_file'].get().strip()
  if v['generate_molecular_figures']:
   for key,ext in [('xyz_file','.xyz'),('potential_dx_file','.dx')]:
    p=Path(v[key])
    if not p.is_file():raise ValueError(f'{key.replace("_"," ")} is required and must exist when molecular figures are enabled')
    if p.suffix.lower()!=ext:raise ValueError(f'{key.replace("_"," ")} must be a {ext} file')
  return v
 ...
 def create_notebook(self):
  try:
   ...
   if v['generate_molecular_figures']:
    inp=self.run_dir/'molecular_inputs'; inp.mkdir()
    for key,name in [('xyz_file','geometry.xyz'),('potential_dx_file','potential.dx')]:
     dst=inp/name; shutil.copy2(v[key],dst); v[key]=str(dst)
